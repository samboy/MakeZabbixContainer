...
######################## SET UP ANSIBLE ########################
# To install ansible on the CentOS7 host, as root:
...
- name: Set up nginx
  ...
  tasks:
    ...
    # Since Docker CentOS7 images are crippled and do not have systemd,
    # we simply directly start nginx.  On a real server, this would use
    # the "service" module instead of the "command" module.
    - name: Start nginx
      command: /usr/sbin/nginx

    # In a real-world setup, we would use the "copy" module.
    - name: Set up nginx home page
      shell: "echo '<h1>Hello, world!</h1>' > /usr/share/nginx/html/index.html"
EOF
echo For this example, password is simply \"password\"
ansible-playbook -v nginx-playbook.yml --ask-become-pass
